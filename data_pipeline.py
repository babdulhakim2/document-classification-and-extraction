...
class DocumentPipeline:
    ...
    def preprocess(self, file) -> str:
        ...
        try:
            ...
            if not file_bytes:
                raise Exception(
                    "File appears empty after read; ensure the file stream is reset and not corrupted."
                )
            ...
            if file.type == "application/pdf":
                ...
                return (
                    f"data:{file.type};base64,{base64.b64encode(file_bytes).decode()}"
                )
        except Exception as e:
        ...
        try:
            with io.BytesIO(image_bytes) as buffer:
                ...
                if (
                    ...
                    > IMAGE_SIZE_THRESHOLDS["large_image_pixels"]
                ):
                    img.thumbnail(
                        IMAGE_SIZE_THRESHOLDS["thumbnail_size"],
    ...
    def _image_to_base64(self, img: Image.Image) -> str:
        ...
        with io.BytesIO() as buffer:
            ...
            if (
                img.size[0] * img.size[1]
                > IMAGE_SIZE_THRESHOLDS["jpeg_threshold_pixels"]
            ):
    ...
    def classify(
        self, file_base64: str, model_name: str, filename: str = "document"
    ) -> DocumentResult:
        ...
        try:
            ...
            return self._parse_classification(
                response, model_name, time.time() - start_time
            )
        except Exception as e:
    ...
    def judge(
        self, file_base64: str, prediction: Dict[str, Any], filename: str = "document"
    ) -> Tuple[float, str]:
        ...
        try:
            ...
            response = self._make_request(
                messages, settings.judge_model, schema=self._get_judge_schema()
            )
            content = response["choices"][0]["message"]["content"]
            logger.info(f"Judge raw response: {str(content)[:200]}...")
            data = self._extract_json(content, context="judge")

            confidence = max(0.0, min(1.0, float(data["confidence"])))
            reasoning = data["reasoning"]

            logger.info(
                f"Judge: confidence={confidence}, reasoning='{reasoning[:50]}...'"
            )
    ...
    def _make_request(
        self, messages: List, model_name: str, schema: Dict = None
    ) -> Dict:
        """Make HTTP request to OpenRouter API with retry"""
        if model_name not in settings.available_models:
            raise ValueError(
                f"Model '{model_name}' not found in available models: {list(settings.available_models.keys())}"
            )
        ...
        # Add PDF processing plugin if messages contain PDF files more on https://openrouter.ai/docs/features/multimodal/pdfs
        has_pdf = any(
            any(
                item.get("type") == "file"
                and item.get("file", {}).get("filename", "").endswith(".pdf")
        ...
        if has_pdf:
            base_payload["plugins"] = [
                {"id": "file-parser", "pdf": {"engine": settings.pdf_engine}}
            ]
        ...
        for attempt in range(settings.max_retries):
            try:
                ...
                if response.status_code >= 500:
                    logger.warning(
                        f"API server error (attempt {attempt + 1}/{settings.max_retries}): {response.status_code}"
                    )
                ...
                last_error = e
                logger.warning(
                    f"Request error (attempt {attempt + 1}/{settings.max_retries}): {str(e)}"
                )
    ...
    def _create_messages(
        self, prompt: str, file_base64: str, filename: str = "document"
    ) -> List[Dict]:
            ...
            content.append(
                {
                    ...
                    "file": {
                        "filename": (
                            filename if filename.endswith(".pdf") else f"{filename}.pdf"
                        ),
        ...
        # Text files (e.g., CSV/TXT): inline decoded text
            ...
            try:
                ...
                if idx != -1:
                    ...
                    decoded = base64.b64decode(txt_b64 or "").decode(
            ...
            content.append(
                {
                    "type": "file",
                    "file": {"filename": filename, "file_data": file_base64},
                }
            )
    ...
    def _parse_classification(
        self, response: Dict, model_name: str, processing_time: float
    ) -> DocumentResult:
        """Parse classification response"""
        try:
            content = response["choices"][0]["message"]["content"]
            logger.info(f"Raw response: {str(content)[:200]}...")
            data = self._extract_json(content, context="classification")
    ...
    def _extract_json(self, content: Any, context: str) -> Dict:
        """Extract JSON object from model message content (dict or string)."""
        ...
        if isinstance(content, str):
            ...
            raise ValueError(f"No JSON found in {context} response: {content[:100]}...")
        raise ValueError(f"Unexpected {context} content type: {type(content)}")

    # === POSTPROCESSING ===

    def postprocess(
        self, results: List[DocumentResult], confidences: List[Tuple[float, str]]
    ) -> List[Dict[str, Any]]:
        """Format final results for output"""
        processed_results = []

        for i, (result, (confidence, reasoning)) in enumerate(
